...
public class DaoUserMySQL extends BasicDao implements IDaoUser{
	...
	private static final String UPDATE_USERS = "UPDATE users SET  name=?, surname=?, phone=?, address=?, image=?, bio=?";
	private static final String UPDATE_USERS_NO_IMAGE = "UPDATE users SET name=?, surname=?, phone=?, address=?, bio=?";
	...
	private User getUserFromMap( Map<String, String> map ) {
		User u = IMappable.fromMap(User.class, map );
		String image = u.getImage();
		if ( image != null ) {
			u.setImage( "img/" + image );
		}
		return u;
	}
	...
	public boolean update(User u) {
		if(u.getImage() != null) {
			return executeAndIsModified( UPDATE_USERS + WHERE_ID,
					u.getName(),
					u.getSurname(),
					u.getPhone(),
					u.getAddress(),
			...
			return executeAndIsModified( UPDATE_USERS_NO_IMAGE + WHERE_ID,
					u.getName(),
					u.getSurname(),
					u.getPhone(),
					u.getAddress(),
